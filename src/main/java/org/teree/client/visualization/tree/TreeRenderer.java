...
public abstract class TreeRenderer<T extends Widget & NodeInterface> {
	 ...
	 * @param root
	 * @param makePicture
	 *            generate the map into canvas to be transformed into picture
	 * @param editable
	 * @return root x,y coordinates
	 */
	protected abstract int[] render(Canvas canvas, List<T> nodes, Node root,
	...
	protected void prepare(List<T> nodes) {
		...
		for (int i = 0; i < nodes.size(); ++i) {
			...
			if (node.getOffsetWidth() == Settings.NODE_MIN_WIDTH
					|| node.getOffsetWidth() == Settings.NODE_MAX_WIDTH + 4
	 ...
	 * @param root
	 * @param leftBounds
	 *            leftBounds.get(col).get(row)
	 * @param rightBounds
	 * @param widths
	 *            [0]-leftw, [1]-rightw
	 */
	protected void setBoundsAndWidths(List<T> nodes, Node root,
			List<List<Integer>> leftBounds, List<List<Integer>> rightBounds,
			Integer[] widths) {
		...
		for (int i = 0; rootcn != null && i < rootcn.size(); ++i) {
			...
			if (n.getLocation() == NodeLocation.LEFT
				...
				BoundsData<T> data = new BoundsData<T>(nodes, leftBounds,
						nc.getOffsetWidth());

				int h = 0;
				if (!nc.isCollapsed()) {
					// don't worry, the current node is set in setBounds
					h = setBounds(data, n.getChildNodes(), nc.getOffsetWidth(),
							data.getMaxWidthLevel(), id + 1);
				}
				...
				BoundsData<T> data = new BoundsData<T>(nodes, rightBounds,
						nc.getOffsetWidth());
	 ...
	 */
	protected int setBounds(BoundsData<T> data, List<Node> childNodes,
	...
	protected void setChildNodesByLocationAndHeights(Node root,
			List<Node> left, List<Node> right, List<List<Integer>> leftBounds,
			List<List<Integer>> rightBounds, Integer[] heights) {
		List<Node> rootcn = root.getChildNodes();
		...
		for (int i = 0; rootcn != null && i < rootcn.size(); ++i) {
			...
			if (n.getLocation() == NodeLocation.LEFT) {
				heights[LEFT] = heights[LEFT]
						+ leftBounds.get(0).get(left.size());
				left.add(n);
			} else {
				heights[RIGHT] = heights[RIGHT]
						+ rightBounds.get(0).get(right.size());
				right.add(n);
			}
		}
	}
	 ...
	 */
	protected int generate(GenerateData<T> data, List<Node> childNodes,
			int startX, int startY, int left, int top, int level,
			int startChildNodeY, int id) {

		int py = startChildNodeY;
		int lvl = 0;

		int margin = (data.getLocation() == NodeLocation.LEFT) ? -MARGIN
				: MARGIN;
		int curveness = (data.getLocation() == NodeLocation.LEFT) ? -CURVENESS
				: CURVENESS;
		...
		for (int i = 0; i < childNodes.size(); ++i) {
			...
			int x = (data.getLocation() == NodeLocation.LEFT) ? startX
					- nw.getOffsetWidth() : startX;
			x += margin;
			lvl = ((data.getLevelBounds().size() > level) ? data
					.getLevelBounds().get(level)
					.get(data.getColumns().get(level)) / 2 : 0);
			int y = lvl + py;
			...
			id++;
			// underline node
			Shapes.drawLine(data.getContext(), x, y + nw.getOffsetHeight() / 2,
			...
			Shapes.drawCurve(
					...
					startY,
					x
							+ ((data.getLocation() == NodeLocation.LEFT) ? nw
									.getOffsetWidth() : 0),
			...
			if (!nw.isCollapsed() && n.getChildNodes() != null
				...
				x = (data.getLocation() == NodeLocation.RIGHT) ? startX
						+ nw.getOffsetWidth() + margin : x; // new startX
				id = generate(data, n.getChildNodes(), x,
						y + nw.getOffsetHeight() / 2, left, top, level + 1, py,
						id);
